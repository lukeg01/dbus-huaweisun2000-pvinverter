...
class ModbusDataCollector2000:
    ...
    def getInverterData(self):
        ...
        if status == "Starting":
            ...
            data['/StatusCode'] = 7
        elif status == "Grid connection: power limited":
            data['/StatusCode'] = 7
        elif status == "Grid connection: self-derating":
            data['/StatusCode'] = 7
        elif status == "Shutdown: fault":
            data['/StatusCode'] = 10
        elif status == "Shutdown: command":
            data['/StatusCode'] = 10
        elif status == "Shutdown: OVGR":
            data['/StatusCode'] = 10
        elif status == "Shutdown: communication disconnected":
            data['/StatusCode'] = 10
        elif status == "Shutdown: power limited":
            data['/StatusCode'] = 10
        elif status == "Shutdown: manual startup required":
            data['/StatusCode'] = 10
        elif status == "Shutdown: DC switches disconnected":
            data['/StatusCode'] = 10
        elif status == "Shutdown: rapid cutoff":
            data['/StatusCode'] = 10
        elif status == "Shutdown: input underpowered":
            data['/StatusCode'] = 10
        elif status == "Standby: no irradiation":
            data['/StatusCode'] = 8
        else:
    ...
    def write_power_limit(self, register_val: int) -> bool:
        """Write ActivePowerPercentageDerating (register 40125). register_val: 0–1000, gain /10 → 0–100.0%."""
        if not self.invSun2000.connect():
            self.logger.error("write_power_limit: not connected")
            return False
        try:
            result = self.invSun2000.inverter.write_register(40125, register_val, unit=self.invSun2000.modbus_unit)
            if hasattr(result, 'isError') and result.isError():
                self.logger.error(f"write_power_limit: Modbus error: {result}")
                return False
            return True
        except Exception as e:
            self.logger.error(f"write_power_limit: exception: {e}")
            return False
