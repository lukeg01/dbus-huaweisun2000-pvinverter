...
class DbusRunServices:
    def __init__(self, services_data, settings, logger, watchdog_fn=None):
...
def NewService(servicename, settings, logger, paths, devicedata, role='pvinverter'):
    ...
    for _path, _settings in paths.items():
        _dbusservice.add_path(
            ...
            onchangecallback=_settings.get('onchangecallback', handlechangedvalue))
...
def main():
    ...
    try:
        ...
        _power_limit_state = {'last_update': None}
        _POWER_LIMIT_TIMEOUT = 60  # seconds before watchdog resets to 100%
        _MAX_POWER_W = 5000  # SUN2000-5KTL-L1 rated power

        def _handle_power_limit(path, value):
            if value is None:
                register_val = 1000
                _power_limit_state['last_update'] = None
            else:
                pct = max(0.0, min(100.0, (float(value) / _MAX_POWER_W) * 100.0))
                register_val = int(pct * 10)
                _power_limit_state['last_update'] = time.time()
            modbus.write_power_limit(register_val)
            logger.info(f"PowerLimit: {value}W → register 40125 = {register_val} ({register_val / 10.0:.1f}%)")
            return True

        def _power_limit_watchdog():
            if _power_limit_state['last_update'] is not None:
                if time.time() - _power_limit_state['last_update'] > _POWER_LIMIT_TIMEOUT:
                    logger.info("PowerLimit watchdog: timeout, resetting to 100%")
                    modbus.write_power_limit(1000)
                    _power_limit_state['last_update'] = None
        ...
        modbus.write_power_limit(1000)
        logger.info("PowerLimit initialized to 100% (register 40125 = 1000)")
        ...
        run_services = DbusRunServices(
            ...
            settings=settings,
            logger=logger,
            watchdog_fn=_power_limit_watchdog
        )
        run_services.run()
